...
run(["aws", "s3", "sync", "templates-sync/templates", "s3://" + s3bucket + "/templates", "--size-only"], check=True)

# SYNC STACKS (raise exception on error)
run(["aws", "s3", "sync", "stacks-sync/stacks", "s3://" + s3bucket + "/stacks", "--size-only"], check=True)
